...
def xmap_to_SA_aln(xmapD,outdir,fname_prefix,ref_cmaps,contig_cmaps):
    ...
    for fD in xmapD:
        ...
        with open(outname,'w') as outfile:
            ...
            outfile.write("#" + seg_id + "\t" + str(score) + "\tFalse\n")
            ...
            for i in alist[:-1]:
                outlist = [contig_id,seg_id,str(i[2]),str(i[1]),"+",orientation,"0","0","0"]
                outfile.write("\t".join(outlist) + "\n")

            #write the last one and include total score
            i = alist[-1]
            outlist = [contig_id,seg_id,str(i[2]),str(i[1]),"+",orientation,"0",str(score),"0"]
            outfile.write("\t".join(outlist))
